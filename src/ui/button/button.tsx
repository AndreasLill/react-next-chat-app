...
const Button = forwardRef<HTMLButtonElement, Props>(({ variant, icon, text, loading, className, onClick, ...props }, forwardedRef) => {
    ...
    return (
        <button
            ...
            ref={forwardedRef}
            className={clsx(
                variant === 'filled' && (props.disabled ? styles.disabled : styles.filled),
                variant === 'outline' && (props.disabled ? styles.disabled : styles.outline),
                variant === 'subtle' && (props.disabled ? styles.disabled : styles.subtle),
